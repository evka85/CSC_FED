...
UHAL_ADDRESS_TABLE_FILE_CTP7='./address_table/uhal_csc_fed_ctp7.xml'
UHAL_ADDRESS_TABLE_FILE_GLIB='./address_table/uhal_csc_fed_glib.xml'
...
VHDL_REG_GENERATED_DISCLAIMER = '(this section is generated by <csc_amc_repo_root>/scripts/generate_registers.py -- do not edit)'
...
def main():
    ...
    if len(sys.argv) < 2:
        print('Usage: generate_registers.py <board_type> [num_of_dmbs]')
        print('board_type can be: ctp7 or glib')
        print('num_of_dmbs is optional. if supplied, the script will substitute generate_size with this value when generating registers in the firmware. Note that this is not affecting the uHAL xml file generation.')
        return
    else:
        ...
        if len(sys.argv) > 2:
            num_of_dmb = parseInt(sys.argv[2])
    ...
    findRegisters(root, '', 0x0, modules, None, vars, False, num_of_dmb)
...
def findRegisters(node, baseName, baseAddress, modules, currentModule, vars, isGenerated, num_of_dmb):
    if (isGenerated == None or isGenerated == False) and node.get('generate') is not None and node.get('generate') == 'true':
        if node.get('generate_idx_var') == 'DMB_IDX':
            generateSize = num_of_dmb
        else:
        ...
        for i in range(0, generateSize):
            ...
            findRegisters(node, baseName, baseAddress + generateAddressStep * i, modules, currentModule, vars, True, num_of_dmb)
        return
    ...
    for child in node:
        findRegisters(child, name, address, modules, module, vars, False, num_of_dmb)
